(function () {
    ...
    (function () {
        ...
        layoutMenuEl.forEach(function (element) {
            menu = new Menu(element, { orientation: "vertical", closeChildren: false });
    ...
    // sortable(".js-grid", { forcePlaceholderSize: true, placeholderClass: "border" });
    // sortable(".o-sortable", { forcePlaceholderSize: true, placeholderClass: "border" });
    try {
        ionRangeSlider(".single-slider");
        document.querySelectorAll(".slider").forEach(function (slider) {
            ionRangeSlider(slider, { min: 0, max: 5000, prefix: "$", grid: true, grid_num: 5, step: 100 });
        });
    } catch (err) {}
    ...
    try {
        ...
        document.getElementById("sweetalert_8").addEventListener("click", function (e) {
            Swal.fire({ title: "Good job!", text: "You clicked the button!", icon: "success", buttonsStyling: false, confirmButtonText: "Confirm me!", customClass: { confirmButton: "btn btn-primary" } });
        });
        ...
        document.getElementById("sweetalert_10").addEventListener("click", function (e) {
            Swal.fire({ position: "top-right", icon: "success", title: "Your work has been saved", showConfirmButton: false, timer: 1500 });
        });
        document.getElementById("sweetalert_11").addEventListener("click", function (e) {
            Swal.fire({ title: "jQuery HTML example", showClass: { popup: "animate__animated animate__fadeInDown" }, hideClass: { popup: "animate__animated animate__fadeOutUp" } });
        });
        document.getElementById("sweetalert_12").addEventListener("click", function (e) {
            Swal.fire({ title: "Are you sure?", text: "You won to be able to revert this!", icon: "warning", showCancelButton: true, confirmButtonText: "Yes, delete it!" }).then(function (result) {
                if (result.value) {
        ...
        document.getElementById("sweetalert_13").addEventListener("click", function (e) {
            Swal.fire({ title: "Are you sure?", text: "You won to be able to revert this!", icon: "warning", showCancelButton: true, confirmButtonText: "Yes, delete it!", cancelButtonText: "No, cancel!", reverseButtons: true }).then(
                function (result) {
                    if (result.value) {
                        Swal.fire("Deleted!", "Your file has been deleted.", "success");
    ...
    //                         showErrorAlert('Error', `Room no "${tagText}" is already selected.`);
    //                     } else {
    //                         createTag(tagText);
    //                         tagInput.value = "";
    //                     }
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        function createTag(tagText) {
            ...
            // Add event listener to remove the tag and corresponding hidden input
            tag.querySelector(".tag-close").addEventListener("click", function () {
                tag.remove();
                removeHiddenInput(tagText);
            });
            
            tagContainer.appendChild(tag);
    
            // Create and append a hidden input with the room number
            const hiddenInput = document.createElement("input");
            hiddenInput.type = "hidden";
            hiddenInput.name = "room_num[]";
            hiddenInput.value = tagText;
            tagContainer.appendChild(hiddenInput);
        }
    
        function removeHiddenInput(tagText) {
            const hiddenInputs = tagContainer.querySelectorAll(`input[name='room_num[]']`);
            for (let input of hiddenInputs) {
                if (input.value === tagText) {
                    input.remove();
                    break;
                }
            }
    ...
    try {
        ...
        if (getEditorId) {
            let quill = new Quill("#editor-container", { modules: { syntax: true, toolbar: "#toolbar-container" }, placeholder: "Write your message here", theme: "snow" });
        }
        const getProductEditorId = document.getElementById("product-editor-container");
        if (getProductEditorId) {
            let quill = new Quill("#product-editor-container", { modules: { syntax: true, toolbar: "#toolbar-container" }, theme: "snow" });
        }
        const getMetaEditorId = document.getElementById("meta-editor-container");
        if (getMetaEditorId) {
            let quill = new Quill("#meta-editor-container", { modules: { syntax: true, toolbar: "#meta-toolbar-container" }, theme: "snow" });
        }
    } catch {}
